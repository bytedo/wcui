...
Renderer.prototype.code = function(code, lang, escaped) {
    ...
    while(idx++ < ln){
        //处理多行注释
        if(!multiCom){
            if(/^\s*?<span class="token comment"/.test(codes[idx - 1])
            ...
            if(!/<\/span>$/.test(codes[idx - 1])){
                codes[idx - 1] = '<span class="token comment">' + codes[idx - 1] + '</span>'
            }else{
...
Renderer.prototype.link = function(href, attr, text) {
    ...
    var out = '<a href="' + href + '" ';
    if (attr) {
        attr = attr.split(';').map(function(it){
            var tmp = it.split('=');
            if(tmp.length > 1){
                return tmp[0] + '="' + tmp[1] +'"'
            }else{
